...
export const Structure = () => {
	...
	return (
		<Stack gap={2}>
			<Typography variant="subtitle2">{__('These settings relate to the structure of your pages.', 'hello-elementor')}</Typography>
			<Setting
				value={headerFooter}
				label={__('Disable theme header and footer', 'hello-elementor')}
				onSwitchClick={() => updateSetting('HEADER_FOOTER', !headerFooter)}
				description={__(
					'What it does: Removes the theme’s default header and footer sections from every page, along with their associated CSS/JS files.',
					'hello-elementor',
				)}
