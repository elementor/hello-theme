...
export default class SettingsPage extends WpAdminPage {
	async gotoSettingsPage(): Promise<void> {
		...
		await this.page.waitForSelector('h4:has-text("Advanced theme settings")', {
			timeout: 10000,
		});
	}
	...
	async clickTab(tabName: string): Promise<void> {
		...
		await this.page.waitForSelector(`[role="tabpanel"]:visible`, {
			timeout: 5000,
		});
	}
	...
	async openChangelog(): Promise<void> {
		...
		await expect(this.page.locator('h4:has-text("Changelog")')).toBeVisible({
			timeout: 5000,
		});
	}
